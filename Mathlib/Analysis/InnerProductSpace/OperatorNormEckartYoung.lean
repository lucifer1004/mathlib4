...
private theorem norm_sq_apply_eq_sum_sq_singularValues (T : E →ₗ[𝕜] F) (z : E) :
  ...
  have hcoord : ∀ i, (inner 𝕜 (b i) ((adjoint T ∘ₗ T) z) : 𝕜)
      = ((T.singularValues i ^ 2 : ℝ) : 𝕜) * inner 𝕜 (b i) z := fun i => by
    rw [← T.isSymmetric_adjoint_comp_self (b i) z, hb,
      T.isSymmetric_adjoint_comp_self.apply_eigenvectorBasis rfl i, inner_smul_left,
      RCLike.conj_ofReal, ← T.sq_singularValues_fin rfl i]
...
private theorem singularValues_mul_norm_le_norm_apply (T : E →ₗ[𝕜] F) (k : ℕ) {z : E}
  ...
  have hsq : (T.singularValues k * ‖z‖) ^ 2 ≤ ‖T z‖ ^ 2 := by
    rw [mul_pow, T.norm_sq_apply_eq_sum_sq_singularValues z,
  ...
  exact (sq_le_sq₀ (mul_nonneg (T.singularValues_nonneg k) (norm_nonneg z)) (norm_nonneg _)).mp hsq
...
private theorem norm_apply_le_singularValues_mul_norm (T : E →ₗ[𝕜] F) (k : ℕ) {z : E}
  ...
  have hsq : ‖T z‖ ^ 2 ≤ (T.singularValues k * ‖z‖) ^ 2 := by
    rw [mul_pow, T.norm_sq_apply_eq_sum_sq_singularValues z,
      ← (T.isSymmetric_adjoint_comp_self.eigenvectorBasis rfl).sum_sq_norm_inner_right z,
      Finset.mul_sum]
    refine Finset.sum_le_sum fun i _ => ?_
    ...
    · exact mul_le_mul_of_nonneg_right
        ...
        (by positivity)
  exact (sq_le_sq₀ (norm_nonneg _) (mul_nonneg (T.singularValues_nonneg k) (norm_nonneg z))).mp hsq
...
theorem eckart_young_spectral_lower (T : E →ₗ[𝕜] F) (k : ℕ) (S : E →ₗ[𝕜] F)
  ...
  · set b := T.isSymmetric_adjoint_comp_self.eigenvectorBasis rfl with hb
    set V : Submodule 𝕜 E :=
      Submodule.span 𝕜 (b '' {i : Fin (finrank 𝕜 E) | (i : ℕ) < k + 1}) with hV
    -- `finrank V = k + 1`.
    have hVdim : finrank 𝕜 V = k + 1 := by
      rw [hV, ← Fin.range_castLE hkn, ← Set.range_comp, finrank_span_eq_card
    ...
    have hcoord : ∀ i : Fin (finrank 𝕜 E), k < (i : ℕ) →
        (inner 𝕜 ((T.isSymmetric_adjoint_comp_self.eigenvectorBasis rfl) i) x : 𝕜) = 0 :=
      fun i hik => inner_eq_zero_of_mem_span b (hV ▸ hxV)
...
theorem eckart_young_spectral_achievable (T : E →ₗ[𝕜] F) (k : ℕ) :
  ...
  have hnormle : ‖x - P x‖ ≤ ‖x‖ :=
    U.starProjection_orthogonal_val x ▸ Uᗮ.norm_starProjection_apply_le x
  ...
  have hcoord : ∀ i : Fin (finrank 𝕜 E), (i : ℕ) < k →
      (inner 𝕜 ((T.isSymmetric_adjoint_comp_self.eigenvectorBasis rfl) i) (x - P x) : 𝕜) = 0 :=
    fun i hik => Submodule.inner_right_of_mem_orthogonal
...
theorem eckart_young_spectral (T : E →ₗ[𝕜] F) (k : ℕ) :
    IsLeast {r : ℝ | ∃ S : E →ₗ[𝕜] F, finrank 𝕜 (LinearMap.range S) ≤ k ∧
      r = ‖(T - S).toContinuousLinearMap‖} (T.singularValues k) :=
  ⟨(eckart_young_spectral_achievable T k).imp fun _ h => ⟨h.1, h.2.symm⟩,
    fun _ ⟨S, hS, hr⟩ => hr.symm ▸ eckart_young_spectral_lower T k S hS⟩
